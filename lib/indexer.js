...
var loop = function (indexer, live, cb) {
  var indexNode = function (data, layer, batch, cb) {
    ...
    var process = function () {
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        batch.push({type: 'put', prefix: indexer.data, key: '!changes!' + layer + '!' + op.dataset + '!' + op.key + '!' + lexint.pack(data.change, 'hex'), value: ptr})
        batch.push({type: 'put', prefix: indexer.data, key: '!latest!' + layer + '!' + op.dataset + '!' + op.key, value: ptr})
      }

      batch.push({type: 'put', prefix: indexer.layers, key: data.key, value: layer})
      batch.push({type: 'put', prefix: indexer.heads, key: layer, value: data.key})
      batch.push({type: 'put', prefix: indexer.meta, key: 'changes', value: data.change.toString()})
      batch.push({type: 'put', prefix: indexer.meta, key: 'status!' + data.key, value: messages.Status.encode(status)})
  ...
  var onmerge = function (data, batch, cb) {
    ...
    batch.push({type: 'put', prefix: indexer.merges, key: headA, value: data.key})
    batch.push({type: 'put', prefix: indexer.merges, key: headB, value: data.key})
  ...
  var oninit = function (data, batch, cb) {
    if (!indexer.mainLayer) {
      ...
      batch.push({type: 'put', prefix: indexer.meta, key: 'layer', value: data.key})
    }
...
var Indexer = function (opts, cb) {
  ...
  var createDb = function () {
    return sublevel(levelup(dbPath, {
      db: opts.backend, prefixEncoding: prefixCodec
    }))
  }
  ...
  if (opts.db) {
    opts.db = sublevel(opts.db, { prefixEncoding: prefixCodec })
