...
module.exports = function (self, opts) {
  ...
  var queue = parallel(16, function (node, cb) {
    ...
    var rows = commit.puts + commit.deletes

    if (rows > 128) {
      throw new Error('chunked batching not yet implemented')
    }
    ...
    hasOperation(commit.operations, function (err, exists) {
      ...
      pump(op, batcher({limit: 128, length: getLength}), through.obj(write), function (err) {
