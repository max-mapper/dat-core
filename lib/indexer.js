...
var Indexer = function (opts, cb) {
  ...
  var createDb = function () {
    var loc = opts.backend ? opts.path : path.join(opts.path, 'db')
    return levelup(loc, {db: opts.backend})
  }

  var once = true
  var ready = function (db, log, leader) {
    self.db = db
    self.blobs = opts.blobs || (opts.path ? cabs({dir: path.join(opts.path, 'blobs')}) : null)
